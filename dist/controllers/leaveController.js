...
exports.approveRejectLeave = exports.getAllLeaves = exports.getPendingLeaves = exports.cancelLeave = exports.getLeaveById = exports.getMyLeaves = exports.createLeave = void 0;
...
const calculateWorkDays = (start, end, workDays = [1, 2, 3, 4, 5]) => {
    let count = 0;
...
const createLeave = async (req, res) => {
    try {
        ...
        const leaveType = await LeaveType_1.default.findById(leaveTypeId);
        if (!leaveType) {
            res.status(404).json({
                success: false,
        ...
        await (0, audit_1.createAuditLog)(userId, 'CREATE', 'Leave', `Applied for ${leaveType.name}`, leave._id);
        res.status(201).json({
            success: true,
...
const getMyLeaves = async (req, res) => {
    try {
        ...
        const leaves = await Leave_1.default.find(filter).populate('leaveTypeId', 'name color code').sort({ createdAt: -1 }).skip((+page - 1) * +limit).limit(+limit);
...
const getPendingLeaves = async (req, res) => {
    try {
        const leaves = await Leave_1.default.find({ status: { $in: ['pending_manager', 'pending_admin'] } })
            .populate('userId', 'firstName lastName email department position')
...
const getAllLeaves = async (req, res) => {
    try {
        ...
        const total = await Leave_1.default.countDocuments(filter);
        const leaves = await Leave_1.default.find(filter).populate('userId', 'firstName lastName email department position').populate('leaveTypeId', 'name color code').sort({ createdAt: -1 }).skip((+page - 1) * +limit).limit(+limit);
        res.json({ success: true, leaves, total, page: +page, totalPages: Math.ceil(total / +limit) });
    }
    catch (error) {
        res.status(500).json({ success: false, message: error.message });
...
const approveRejectLeave = async (req, res) => {
    try {
        ...
        const leave = await Leave_1.default.findById(req.params.id).populate('userId');
        if (!leave) {
            res.status(404).json({ success: false, message: 'Leave not found' });
            return;
        }
        const reviewerRole = req.user.role;
        const leaveUser = leave.userId;
        ...
        if (approvalLevels === 1) {
            if (leave.status !== 'pending_manager') {
                res.status(400).json({ success: false, message: 'Leave is not in pending state' });
                return;
            }
            leave.status = action;
            ...
            await leave.save();
            if (action === 'approved') {
                await User_1.default.findByIdAndUpdate(leave.userId, {
            ...
            await Notification_1.default.create({ userId: leaveUser._id, title: `Leave ${action}`, message: `Your leave request has been ${action}.${comment ? ' Comment: ' + comment : ''}`, type: action === 'approved' ? 'leave_approved' : 'leave_rejected', leaveId: leave._id });
            const dateStr = `${leave.startDate.toDateString()} - ${leave.endDate.toDateString()}`;
            try {
                await (0, email_1.sendLeaveStatusEmail)(leaveUser.email, action, dateStr, comment);
            }
            catch (_) { }
            await (0, audit_1.createAuditLog)(req.user._id, action.toUpperCase(), 'Leave', `Leave ${action} for ${leaveUser.email}`, leave._id);
        ...
        if (approvalLevels === 2) {
            ...
            if (action === 'rejected') {
                if (!['pending_manager', 'pending_admin'].includes(leave.status)) {
                    res.status(400).json({ success: false, message: 'Leave cannot be rejected at this stage' });
                    return;
                }
                ...
                leave.adminComment = comment;
                await leave.save();
                await Notification_1.default.create({ userId: leaveUser._id, title: 'Leave Rejected', message: `Your leave has been rejected.${comment ? ' Reason: ' + comment : ''}`, type: 'leave_rejected', leaveId: leave._id });
                const dateStr = `${leave.startDate.toDateString()} - ${leave.endDate.toDateString()}`;
                try {
                    await (0, email_1.sendLeaveStatusEmail)(leaveUser.email, 'rejected', dateStr, comment);
                }
                catch (_) { }
                await (0, audit_1.createAuditLog)(req.user._id, 'REJECTED', 'Leave', `Leave rejected for ${leaveUser.email}`, leave._id);
            ...
            if (leave.status === 'pending_manager') {
                if (reviewerRole === 'manager') {
                    ...
                    for (const admin of admins) {
                        await Notification_1.default.create({ userId: admin._id, title: '⏳ Final Approval Needed', message: `${leaveUser.firstName} ${leaveUser.lastName}'s leave was approved by manager. Your final approval is required.`, type: 'leave_applied', leaveId: leave._id });
                    }
                    // Notify employee of intermediate progress
                    await Notification_1.default.create({ userId: leaveUser._id, title: '✅ Manager Approved — Awaiting Admin', message: 'Your leave was approved by your manager. Waiting for admin final approval.', type: 'leave_approved', leaveId: leave._id });
                    await (0, audit_1.createAuditLog)(req.user._id, 'APPROVED', 'Leave', `Manager Level-1 approved for ${leaveUser.email}`, leave._id);
                ...
                if (reviewerRole === 'admin') {
                    ...
                    await Notification_1.default.create({ userId: leaveUser._id, title: 'Leave Approved ✅', message: `Your leave has been fully approved.${comment ? ' Comment: ' + comment : ''}`, type: 'leave_approved', leaveId: leave._id });
                    const dateStr = `${leave.startDate.toDateString()} - ${leave.endDate.toDateString()}`;
                    try {
                        await (0, email_1.sendLeaveStatusEmail)(leaveUser.email, 'approved', dateStr, comment);
                    }
                    catch (_) { }
                    await (0, audit_1.createAuditLog)(req.user._id, 'APPROVED', 'Leave', `Admin direct approval for ${leaveUser.email}`, leave._id);
            ...
            if (leave.status === 'pending_admin') {
                if (reviewerRole !== 'admin') {
                    res.status(403).json({ success: false, message: 'This leave is awaiting admin final approval. Only an admin can approve it now.' });
                    return;
                }
                leave.status = 'approved';
                leave.reviewedBy = req.user._id;
                ...
                await Notification_1.default.create({ userId: leaveUser._id, title: 'Leave Fully Approved ✅', message: `Your leave has been fully approved by admin.${comment ? ' Comment: ' + comment : ''}`, type: 'leave_approved', leaveId: leave._id });
                const dateStr = `${leave.startDate.toDateString()} - ${leave.endDate.toDateString()}`;
                try {
                    await (0, email_1.sendLeaveStatusEmail)(leaveUser.email, 'approved', dateStr, comment);
                }
                catch (_) { }
                await (0, audit_1.createAuditLog)(req.user._id, 'APPROVED', 'Leave', `Admin final approval for ${leaveUser.email}`, leave._id);
